...
export default function ComposePage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      {/* Toast */}
      {toast && (
        <div className="fixed top-4 right-4 z-50 rounded-lg bg-brand-500 px-4 py-2.5 text-sm text-white shadow-lg">
          {toast}
        </div>
      )}

      <div className="mb-6 flex items-center justify-between">
        <button onClick={() => router.back()} className="text-sm text-brand-500 hover:text-brand-700">
          &larr; Back
        </button>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  disabled={isSent}
                  className="w-full rounded-lg border border-brand-100 px-3 py-2 text-sm focus:border-brand-400 focus:outline-none focus:ring-1 focus:ring-brand-400 disabled:bg-brand-50/30"
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  disabled={isSent}
                  rows={16}
                  className="w-full rounded-lg border border-brand-100 px-3 py-2 text-sm leading-relaxed focus:border-brand-400 focus:outline-none focus:ring-1 focus:ring-brand-400 disabled:bg-brand-50/30 font-mono"
                />
              </div>
            </div>
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={handleSave}
                    disabled={saving}
                    className="rounded-lg border border-brand-100 px-4 py-2 text-sm font-medium hover:bg-brand-50/50 disabled:opacity-50"
                  >
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="rounded-xl border border-brand-100 bg-white p-5">
            ...
            <div className="space-y-2 text-sm">
              ...
              {contact.linkedinUrl && (
                <a href={contact.linkedinUrl} target="_blank" rel="noopener" className="text-brand-500 text-xs hover:underline block">
                  LinkedIn Profile
                </a>
              )}
            </div>
            ...
            {threads.length === 0 ? (
              ...
              <div className="space-y-3 max-h-80 overflow-y-auto">
                {threads.slice(0, 5).map((thread) => (
                  <div key={thread.id} className="border-b border-brand-50 pb-2 last:border-0">
